...
async function getPublicKey(username: string): Promise<any> {
  // Check cache first
  const cachedKey = publicKeyCache[username];
  const now = Date.now();
  
  if (cachedKey && cachedKey.expires > now) {
...
export async function verifySignedMessage(signedMessage: SignedMessage): Promise<boolean> {
  try {
    ...
    const publicKeyJwk = await getPublicKey(signedMessage.sender_username);
